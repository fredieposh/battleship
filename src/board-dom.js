...
export {loadBoards, getCurrentComputerBoard};
...
function getCurrentComputerBoard() {
    return currentComputerBoard;
}

function switchComputerBoards(otherCurrentComputerBoard) {
    if (currentComputerBoard === 'real') {
        currentComputerBoard = 'dummy';
        emptyComputerContainer();
        loadBoardToComputerContainer(dummyBoard);
        console.log(currentComputerBoard);
        return;
    }
...
function createDummyBoardTileDiv(row, column) {
    const newTile = document.createElement('div');
    newTile.classList.add('board-tile-dummy');
    newTile.setAttribute('id',`dummy-${row}-${column}`);
    return newTile;
};
...
function populateDummyBoardWithTiles() {
    for(let i = 0; i < 10; i++) {
        for(let j = 0; j < 10; j ++) {
            const newTile = createDummyBoardTileDiv(i,j);
...
function changeTileColor(obj) {
    ...
    const selectedDummyTile = dummyBoard.querySelector(`#dummy-${row}-${col}`);
    
    if(user === 'computer') {
        clearTileClasslist(selectedTile);
        selectedTile.removeEventListener('click', boardDomHit);
        selectedDummyTile.classList.add(reason);
    };
